...
现有代码已经有 LAN 房间发现、创建、加入、准备与开始流程，但“开始游戏”之后仍进入各端本地 Demo；正式房主权威 Match Session 尚未实施。
...
3. 正式 LAN Match 仍标记为持续设计中，而实施计划已按 M1～M8 分解。开始对应里程碑前需要确认设计状态，并把与 `SPEC.md` 的冲突先回写，不能由实现自行选择。
