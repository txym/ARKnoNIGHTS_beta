...
function ConvertTo-MarkdownCell {
    ...
    $text = ConvertTo-InvariantText -Value $Value
    return ($text -replace '\r?\n', ' ' -replace '\|', '\|')
}

function Add-MarkdownTableHeader {
    param(
...
function ConvertTo-UnitCostAnalysisMarkdown {
    param(
        [Parameter(Mandatory = $true)][object[]]$Rows,
        [Parameter(Mandatory = $true)]$Analysis
    )

    Assert-Condition ($Rows.Count -eq 94) "Formal report requires 94 rows; found $($Rows.Count)."
    $orderedRows = @($Rows | Sort-Object { [int]$_.TypeId })
    Assert-Condition ((@($Rows | ForEach-Object { [int]$_.TypeId }) -join '/') -ceq (@($orderedRows | ForEach-Object { [int]$_.TypeId }) -join '/')) 'Formal report rows must be in stable numeric TypeId order.'

    $lines = [System.Collections.Generic.List[string]]::new()
    $lines.Add('# Unit Cost dataset analysis (2026-07-28)')
    $lines.Add('')
    $lines.Add('This report is generated by `Export-UnitCostDataset.ps1` from the current BONDS shop set, the approved ability-input data, and the read-only staged unit JSON. The CSV and this report are generated artifacts; neither contains hand-edited unit values.')
    $lines.Add('')
    $lines.Add('## 1. Data integrity')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('Check', 'Result')
    Add-MarkdownTableRow -Lines $lines -Values @('Shop rows', $Rows.Count)
    Add-MarkdownTableRow -Lines $lines -Values @('Unique TypeIds', @($Rows.TypeId | Sort-Object -Unique).Count)
    Add-MarkdownTableRow -Lines $lines -Values @('Numeric TypeId ordering', 'ascending and stable')
    Add-MarkdownTableRow -Lines $lines -Values @('Rarity distribution R1/R2/R3/R4/R5/R6', '9/20/14/24/21/6')
    Add-MarkdownTableRow -Lines $lines -Values @('Actually read resource directories', $Analysis.EliteModel.ActuallyReadResourceDirectoryCount)
    Add-MarkdownTableRow -Lines $lines -Values @('Actually read JSON files', $Analysis.EliteModel.ActuallyReadJsonFileCount)
    Add-MarkdownTableRow -Lines $lines -Values @('Non-shop rows exported', @($Rows | Where-Object { [int]$_.TypeId -in @(1137, 1138, 2033, 5504, 10002) }).Count)
    $lines.Add('')
    $lines.Add('The formal verification independently reruns the exporter into `Temp`, validates the current BONDS shop TypeId set and complete column list, and compares normalized UTF-8 CSV text with this formal CSV.')
    $lines.Add('')

    $rarityAudit = @{}
    foreach ($rarity in 1..6) {
        $rarityRows = @($Rows | Where-Object { [int]$_.Rarity -eq $rarity })
        $costs = [decimal[]]@($rarityRows | ForEach-Object { [decimal]$_.FinalBaseCost })
        $powers = [decimal[]]@($rarityRows | ForEach-Object { [decimal]$_.ContinuousPower })
        $rarityAudit[$rarity] = [pscustomobject]@{
            Count = $rarityRows.Count
            RawPowerMedian = Get-Median -Values $powers
            IsotonicPowerMedian = [decimal]$rarityRows[0].IsotonicRarityMedianPower
            CostMinimum = [decimal](@($costs | Measure-Object -Minimum).Minimum)
            CostP25 = Get-Percentile -Values $costs -Percentile ([decimal]0.25)
            CostMedian = Get-Median -Values $costs
            CostP75 = Get-Percentile -Values $costs -Percentile ([decimal]0.75)
            CostMaximum = [decimal](@($costs | Measure-Object -Maximum).Maximum)
            CostMean = [decimal](@($costs | Measure-Object -Sum).Sum) / [decimal]$costs.Count
        }
    }

    $lines.Add('## 2. R2-R6 adjacent power and Cost growth')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('Transition', 'Previous isotonic power', 'Current isotonic power', 'Power growth', 'Power growth %', 'Previous median Cost', 'Current median Cost', 'Cost growth', 'Cost growth %')
    foreach ($rarity in 3..6) {
        $previous = $rarityAudit[$rarity - 1]
        $current = $rarityAudit[$rarity]
        $powerGrowth = [decimal]$current.IsotonicPowerMedian - [decimal]$previous.IsotonicPowerMedian
        $costGrowth = [decimal]$current.CostMedian - [decimal]$previous.CostMedian
        Add-MarkdownTableRow -Lines $lines -Values @(
            "R$($rarity - 1)->R$rarity",
            $previous.IsotonicPowerMedian,
            $current.IsotonicPowerMedian,
            $powerGrowth,
            (($powerGrowth / [decimal]$previous.IsotonicPowerMedian) * [decimal]100),
            $previous.CostMedian,
            $current.CostMedian,
            $costGrowth,
            (($costGrowth / [decimal]$previous.CostMedian) * [decimal]100)
        )
    }
    $r6R2CostRatio = [decimal]$rarityAudit[6].CostMedian / [decimal]$rarityAudit[2].CostMedian
    $lines.Add('')
    $lines.Add("R6/R2 median final Cost ratio: **$(ConvertTo-InvariantText $r6R2CostRatio)** (R6=$(ConvertTo-InvariantText $rarityAudit[6].CostMedian), R2=$(ConvertTo-InvariantText $rarityAudit[2].CostMedian)); the accepted limit is 4.")
    $lines.Add('')

    $calibration = $Analysis.CostModel.R1Calibration
    $lines.Add('## 3. R1 kappa and 72-match calibration')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('Kappa', 'R1 wins', 'R2 wins', 'Draws', 'R1 score rate', 'Matches', 'Event cap')
    Add-MarkdownTableRow -Lines $lines -Values @($calibration.Kappa, $calibration.Rarity1Wins, $calibration.Rarity2Wins, $calibration.Draws, $calibration.Rarity1WinRate, $calibration.MatchCount, $calibration.MatchEventLimit)
    $lines.Add('')
    $lines.Add('The score rate is `(R1 wins + 0.5 * draws) / 72`; each match below is part of the selected uniform-kappa audit.')
    $lines.Add('')
    $lines.Add('### All 72 match details')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('#', 'Budget', 'R1 formation', 'R2 formation', 'Direction', 'R1 spent', 'R2 spent', 'Winner', 'Resolution', 'Events', 'R1 remaining entities', 'R2 remaining entities', 'R1 remaining target value', 'R2 remaining target value')
    $lines.Add('<!-- R1_MATCH_ROWS_START -->')
    $matchIndex = 0
    foreach ($match in @($calibration.SelectedMatches)) {
        $matchIndex++
        Add-MarkdownTableRow -Lines $lines -Values @(
            $matchIndex, $match.Budget, $match.Rarity1FormationKind, $match.Rarity2FormationKind,
            $match.Direction, $match.Rarity1SpentCost, $match.Rarity2SpentCost, $match.Winner,
            $match.Resolution, $match.EventCount, $match.R1RemainingEntityCount,
            $match.R2RemainingEntityCount, $match.R1RemainingTargetValue, $match.R2RemainingTargetValue
        )
    }
    $lines.Add('<!-- R1_MATCH_ROWS_END -->')
    $lines.Add('')

    $allCosts = [decimal[]]@($Rows | ForEach-Object { [decimal]$_.FinalBaseCost })
    $overallAudit = [pscustomobject]@{
        Count = $Rows.Count
        CostMinimum = [decimal](@($allCosts | Measure-Object -Minimum).Minimum)
        CostP25 = Get-Percentile -Values $allCosts -Percentile ([decimal]0.25)
        CostMedian = Get-Median -Values $allCosts
        CostP75 = Get-Percentile -Values $allCosts -Percentile ([decimal]0.75)
        CostMaximum = [decimal](@($allCosts | Measure-Object -Maximum).Maximum)
        CostMean = [decimal](@($allCosts | Measure-Object -Sum).Sum) / [decimal]$allCosts.Count
    }
    $lines.Add('## 4. Cost distributions')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('Population', 'Count', 'Min', 'P25', 'Median', 'P75', 'Max', 'Mean')
    Add-MarkdownTableRow -Lines $lines -Values @('Overall', $overallAudit.Count, $overallAudit.CostMinimum, $overallAudit.CostP25, $overallAudit.CostMedian, $overallAudit.CostP75, $overallAudit.CostMaximum, $overallAudit.CostMean)
    foreach ($rarity in 1..6) {
        $audit = $rarityAudit[$rarity]
        Add-MarkdownTableRow -Lines $lines -Values @("R$rarity", $audit.Count, $audit.CostMinimum, $audit.CostP25, $audit.CostMedian, $audit.CostP75, $audit.CostMaximum, $audit.CostMean)
    }
    $lines.Add('')
    $lines.Add('### Same-tier low/high ContinuousPower outliers')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('Rarity', 'Side', 'TypeId', 'Display name', 'ContinuousPower', 'Final Cost', 'Within-tier factor', 'RiskFlags')
    foreach ($rarity in 1..6) {
        $tierRows = @($Rows | Where-Object { [int]$_.Rarity -eq $rarity } | Sort-Object { [decimal]$_.ContinuousPower }, { [int]$_.TypeId })
        foreach ($entry in @(
                [pscustomobject]@{ Side = 'Low'; Row = $tierRows[0] },
                [pscustomobject]@{ Side = 'High'; Row = $tierRows[-1] }
            )) {
            Add-MarkdownTableRow -Lines $lines -Values @($rarity, $entry.Side, $entry.Row.TypeId, $entry.Row.DisplayName, $entry.Row.ContinuousPower, $entry.Row.FinalBaseCost, $entry.Row.WithinTierFactor, $entry.Row.RiskFlags)
        }
    }
    $lines.Add('')

    $highCostRows = @($Rows | Where-Object { [int]$_.FinalBaseCost -gt 30 } | Sort-Object { [int]$_.TypeId })
    Assert-Condition ($highCostRows.Count -eq 8) "Formal report requires exactly eight Cost > 30 rows; found $($highCostRows.Count)."
    $lines.Add('## 5. Cost > 30 audit')
    $lines.Add('')
    $lines.Add('Every row is explained by the exported formula and cites the exact quantified/BONDS-confirmed evidence used by the model. Unquantified effects remain explicit risks and do not receive hidden numeric value.')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('TypeId', 'Display name', 'Rarity', 'Final Cost', 'Formula reason', 'Quantified/BONDS-confirmed evidence', 'Unquantified risk')
    $lines.Add('<!-- HIGH_COST_ROWS_START -->')
    foreach ($row in $highCostRows) {
        $powerRatio = [decimal]$row.ContinuousPower / [decimal]$row.IsotonicRarityMedianPower
        $reason = "round($(ConvertTo-InvariantText $row.RarityBaseCost) * $(ConvertTo-InvariantText $row.WithinTierFactor)) = $($row.FinalBaseCost); ContinuousPower $(ConvertTo-InvariantText $row.ContinuousPower) / tier isotonic median $(ConvertTo-InvariantText $row.IsotonicRarityMedianPower) = $(ConvertTo-InvariantText $powerRatio); model $($row.AbilityModelKind)"
        $quantifiedEvidence = if ([string]$row.AbilityModelKind -ceq 'None') {
            "Canonical staged level-0 panel: HP $($row.MaxHitPoints), ATK $($row.Attack), DEF $($row.Defense), MR $($row.MagicResistance), $($row.DamageType) damage, effective interval $(ConvertTo-InvariantText $row.EffectiveAttackIntervalSeconds)s; no BONDS ability modifier is claimed."
        }
        else {
            [string]$row.AbilityEvidence
        }
        Add-MarkdownTableRow -Lines $lines -Values @($row.TypeId, $row.DisplayName, $row.Rarity, $row.FinalBaseCost, $reason, $quantifiedEvidence, $row.RiskFlags)
    }
    $lines.Add('<!-- HIGH_COST_ROWS_END -->')
    $lines.Add('')

    $eliteRiskRows = @($Rows | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_.EliteEfficiencyRisk) } | Sort-Object { [int]$_.TypeId })
    Assert-Condition ($eliteRiskRows.Count -eq 80) "Formal report requires 80 elite-efficiency risks; found $($eliteRiskRows.Count)."
    $lines.Add('## 6. Elite efficiency risks')
    $lines.Add('')
    $lines.Add('C0 is unchanged. Dedicated-stage efficiency deltas and BONDS E2 ability-reuse limitations are listed for all 80 risk-bearing rows.')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('TypeId', 'Display name', 'E0 power/Cost', 'E2 source', 'E2 power/Cost', 'E3 source', 'E3 power/Cost', 'Risk')
    $lines.Add('<!-- ELITE_RISK_ROWS_START -->')
    foreach ($row in $eliteRiskRows) {
        Add-MarkdownTableRow -Lines $lines -Values @($row.TypeId, $row.DisplayName, ([decimal]$row.ContinuousPower / [decimal]$row.FinalBaseCost), $row.Elite2ResourceSource, $row.Elite2PowerPerCostRatio, $row.Elite3ResourceSource, $row.Elite3PowerPerCostRatio, $row.EliteEfficiencyRisk)
    }
    $lines.Add('<!-- ELITE_RISK_ROWS_END -->')
    $lines.Add('')

    $riskRows = @($Rows | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_.RiskFlags) } | Sort-Object { [int]$_.TypeId })
    $lines.Add('## 7. Unquantified abilities')
    $lines.Add('')
    $lines.Add('All ability effects that remain partly or wholly unquantified are preserved as risk text. `ExplicitRiskOnly` rows receive zero numeric ability contribution.')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('TypeId', 'Display name', 'Ability model', 'Quantified evidence', 'Unquantified ability/risk')
    $lines.Add('<!-- UNQUANTIFIED_ROWS_START -->')
    foreach ($row in $riskRows) {
        Add-MarkdownTableRow -Lines $lines -Values @($row.TypeId, $row.DisplayName, $row.AbilityModelKind, $row.AbilityEvidence, $row.RiskFlags)
    }
    $lines.Add('<!-- UNQUANTIFIED_ROWS_END -->')
    $lines.Add('')

    $economyRows = @($Analysis.EconomyPressure.Rows)
    Assert-Condition ($economyRows.Count -eq 144) "Formal report requires 144 economy rows; found $($economyRows.Count)."
    $lines.Add('## 8. Economy pressure grid')
    $lines.Add('')
    $lines.Add('This is a pressure check only. **Permanent Cost income is unconfirmed.** Quantity is `min(floor(GoldBudget / rarity), floor(CostBudget / rarity median final Cost))`.')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('Gold budget', 'Cost budget', 'Rarity', 'Median Cost', 'Gold-limited quantity', 'Cost-limited quantity', 'Quantity', 'Limiter')
    $lines.Add('<!-- ECONOMY_ROWS_START -->')
    foreach ($economyRow in $economyRows) {
        Add-MarkdownTableRow -Lines $lines -Values @($economyRow.GoldBudget, $economyRow.CostBudget, $economyRow.Rarity, $economyRow.MedianCost, $economyRow.GoldLimitedQuantity, $economyRow.CostLimitedQuantity, $economyRow.Quantity, $economyRow.Limiter)
    }
    $lines.Add('<!-- ECONOMY_ROWS_END -->')
    $lines.Add('')
    $lines.Add('### Quantity ratios')
    $lines.Add('')
    Add-MarkdownTableHeader -Lines $lines -Headers @('Gold budget', 'Cost budget', 'R1/R2 quantity ratio', 'R2/R6 quantity ratio')
    foreach ($ratio in @($Analysis.EconomyPressure.QuantityRatios)) {
        Add-MarkdownTableRow -Lines $lines -Values @($ratio.GoldBudget, $ratio.CostBudget, $ratio.R1ToR2, $ratio.R2ToR6)
    }
    $lines.Add('')

    $lines.Add('## 9. Verification scope')
    $lines.Add('')
    $lines.Add('- Permanent Cost income is unconfirmed; the economy grid is not a claim about live-match income.')
    $lines.Add('- Unity compilation was not run.')
    $lines.Add('- Unity EditMode tests were not run.')
    $lines.Add('- Unity PlayMode tests were not run.')
    $lines.Add('- A Unity player/platform build was not run.')
    $lines.Add('- The standalone PowerShell verification checks the formal CSV against a fresh independent Temp export and verifies BONDS, ability-input, and the 362 staged JSON files remain unchanged.')

    $result = [string[]]$lines.ToArray()
    Assert-Condition ($result.Count -gt 0 -and $result[0] -ceq '# Unit Cost dataset analysis (2026-07-28)') 'Formal report must start with the deterministic title.'
    foreach ($line in $result) {
        Assert-Condition ($line -is [string]) 'Formal report output must contain strings only.'
    }
    return $result
}
...
try {
    ...
    try {
        ...
        if ([System.IO.Path]::GetExtension($AnalysisOutputPath) -ieq '.md') {
            Write-Utf8File -Path $analysisTemporaryPath -Lines @(ConvertTo-UnitCostAnalysisMarkdown -Rows $rows -Analysis $analysis)
        }
        else {
