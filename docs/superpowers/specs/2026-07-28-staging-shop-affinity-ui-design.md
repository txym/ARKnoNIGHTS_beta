...
- 单位映射：`TypeId`、可选的种类 ID、可选的地区 ID，但两者不能同时为空。

当前 BONDS 已把每个商店单位归入至多一个地区。无地区单位的地区 ID 为空；当前 8 个仅有地区归属的单位，其种类 ID 为空。“其他”种类保留中文名称，但图标路径为空。
...
1. BONDS 中 94 个正式商店单位全部存在唯一运行时映射；
2. 当前 Demo 仍引用但已从 BONDS 正式名单移除的 `1000`，仅保留一条显式兼容映射：`整合运动 + 感染生物`；
3. 每个正式映射至少有地区或种类之一，且 81 个地区归属、86 个种类归属与 BONDS 一致；
4. 最新 BONDS 中不存在多地区商店单位；
5. 五个有图标种类和八个地区的资源能够加载；
6. “其他”的空图标路径不会被当作错误；
7. HeaderLeft 对有地区单位优先显示地区图标；
8. HeaderLeft 对无地区单位回退到种类图标；
9. 商店三行顺序、层级、字体、Sprite 名、`preserveAspect` 和中心对齐正确；
10. 商店 cost 显示部署费用而不是购买价格；
11. 无地区单位隐藏地区行；无种类单位隐藏种类行；“其他”保留文字并隐藏图标；
12. 空槽位隐藏三行，不残留上一商品的数据；
13. 既有 Ready 按钮、购买、刷新、冻结、升级和待部署交互测试无回归。
