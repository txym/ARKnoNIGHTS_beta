...
`BattleTrackPlaybackController` 只为切入 Tick 仍存活且应该显示的单位创建视图。已存在的视图在连续播放跨过 Death 时仍收到一次 `PlayDeath`。`UnitSkelPresentationView` 监听实际 Spine `TrackEntry.Complete`，完成后自行执行 `0.5` 秒变黑并隐藏。
