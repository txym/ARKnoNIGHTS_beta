...
> 本文件保留历次任务的测试命令、结果与证据位置，不代表当前工作树已经重新验证。
> 当前验证方法与完成标准请阅读 [`docs/TEST_PLAN.md`](../TEST_PLAN.md)。
...
## 53. LAN Match M2 共享牌库、商店、经济与升级（2026-07-30）

- 实现分支为 `codex/lan-match-economy`，独立 worktree 为 `G:\ARKnoNIGHTS_beta\.worktrees\lan-match-economy`，实际分支点为 `19a4f62d124ef79823a169bc604998352e123588`；`c615a75` 已确认是该分支点祖先。没有修改 Scene、Prefab、Package、`LocalMatchState`、Lobby Socket 或 Battle 实现。
- 修改前基线命令使用 `scripts/Invoke-UnityTests.ps1`、Unity `2022.3.62f1c1`、过滤器 `ArknoNights.Match.Tests`，结果为 `21 total / 21 passed / 0 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`；证据目录为 `Artifacts/LanMatchEconomy/Baseline-M1`。
- TDD 红测分别保留在 `Artifacts/LanMatchEconomy/Red-Initialization`、`Red-RefreshFreeze` 与 `Red-PurchaseUpgrade`。前两次因待实现类型缺失而在编译阶段按预期失败；第三次明确报告缺少 `PurchaseShopOfferCommand`、`PurchaseLevelUpgradeCommand` 和可替换容量策略入口。红测结果不计为通过。
- 最终 Match 编译命令为 `dotnet build ARKnoNIGHTS.Match.EditModeTests.csproj --no-restore -v:minimal`，退出码为 `0`、编译错误为 `0`。生成的 Unity `.csproj` 报告两个既有程序集版本选择警告：`System.Net.Http` 与 `System.Security.Cryptography.Algorithms`；Unity focused 日志没有 `error CS`、`Scripts have compiler errors`、`Compilation failed` 或未处理异常。
- 最终 focused Match EditMode 命令使用过滤器 `ArknoNights.Match.Tests`，结果为 `60 total / 60 passed / 0 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`。其中 M1 既有四个 fixture 合计 `21/21`；M2 新增目录/初始化 `9/9`、PRNG `5/5`、刷新/冻结 `13/13`、购买/升级 `12/12`，合计 `39/39`。结果位于 `Artifacts/LanMatchEconomy/Final-Match-EditMode-Final/EditModeResults.xml`，日志位于同目录 `EditMode.log`；完整 XML 写出后 wrapper 在 20 秒退出宽限结束时执行 `forced-stop-after-results`，不是超时、跳过或结果缺失。
- 最终全量 EditMode 使用同一脚本且不传 `TestFilter`，得到 `463 total / 458 passed / 5 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`，Unity 在结果写出后正常退出；证据目录为 `Artifacts/LanMatchEconomy/Final-Full-EditMode-Final`。五项失败与 M1 的 `424/419` 基线逐项相同：真实 Battle 动态果冻期望 `27`、实际 `24`；`5503` 精英 0 旧技能描述期望为空、实际源非空；三项 `UnitSourceConsumerEditModeTests` 继续因 Windows CRLF 得到 `BE09...` 而不等于 LF 冻结哈希 `359C...`。失败列表没有 Match 测试，因此不能把全量记为通过，但没有本次 M2 引入的新失败。
- 本轮没有运行 PlayMode、Windows/Android 构建、Socket、场景、UI 或设备验证。M2 是 `noEngineReferences=true` 的纯 C# 领域扩展，任务验收只要求相关 C# 编译、M1/M2 focused EditMode 与可用的全量 EditMode；不得由这些证据推断正式 LAN 对局、当前 Demo UI 或跨端协议已经接入。

## 54. LAN Match M3 自动合成与 Overflow（2026-07-30）

- 实现分支为 `codex/lan-match-fusion`，独立 worktree 为 `G:\ARKnoNIGHTS_beta\.worktrees\lan-match-fusion`；M2 直接基线为 `1d62ee713aa17ce24d2bc2098516d9c7eea55fed`，M1 merge-base 为 `c615a752c324c836b18dfabfbe553943b5f02f5d`。没有修改 Scene、Prefab、Package、`LocalMatchState`、Lobby Socket、Battle Core 或表现资源。
- 修改前 focused Match EditMode 使用 Unity `D:\2022.3.62f1c1\Editor\Unity.exe`、项目脚本 `scripts/Invoke-UnityTests.ps1` 和过滤器 `ArknoNights.Match.Tests`，结果为 `60 total / 60 passed / 0 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`；结果位于 `Temp/UnityTests/LanMatchFusion-Baseline/EditModeResults.xml`。
- TDD Red 在 `Temp/UnityTests/LanMatchFusion-Red/EditMode.log` 按预期因缺少 `PlayerTargetedUnitBuffState`、`PlayerGlobalBuffState` 和 `PlayerSourceEffectState` 而编译失败；该轮没有结果 XML，不计为通过。
- 最终程序集编译命令为 `dotnet build .\ARKnoNIGHTS.Match.EditModeTests.csproj --no-restore -v:minimal`，退出码 `0`、编译错误 `0`；只保留 M2 已记录的 `System.Net.Http` 与 `System.Security.Cryptography.Algorithms` 两类 Unity 生成工程引用版本选择警告。
- 最终 focused Match EditMode 命令为 `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\Invoke-UnityTests.ps1 -UnityPath D:\2022.3.62f1c1\Editor\Unity.exe -ProjectPath G:\ARKnoNIGHTS_beta\.worktrees\lan-match-fusion -TestPlatform EditMode -TestFilter ArknoNights.Match.Tests -OutputDirectory Temp\UnityTests\LanMatchFusion-FinalFocused3 -TimeoutSeconds 900 -NoGraphics`。结果为 `87 total / 87 passed / 0 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`；XML 与日志分别位于该目录的 `EditModeResults.xml` 和 `EditMode.log`。完整 XML 写出后 wrapper 执行 `forced-stop-after-results`，不是超时或结果缺失。
- M3 focused 用例覆盖两副本与四/八副本连锁、MaxElite、候选区与幸存规则、幂等诊断、永久退休、玩家层定向 Buff、Cost 保持/退场、严格堆叠与排序、购买前 13 槽、非商店授权获得、Overflow 非阻塞提升、封存删除和权限投影；M1/M2 全部既有 Match 用例同时通过。
- 第一次最终全量 EditMode 在结果写入期间触发 wrapper 的 XML 读取竞态，`Temp/UnityTests/LanMatchFusion-FinalFullEditMode` 被标记为 `unverified / forced-stop-before-valid-results`，不得计为通过或失败。确认无 Unity 进程占用后换新目录重跑。
- 有效的最终全量 EditMode 不传 `TestFilter`，结果为 `490 total / 485 passed / 5 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`，Unity 在 XML 写出后正常退出；证据位于 `Temp/UnityTests/LanMatchFusion-FinalFullEditMode2`。五项失败与 M2 的 `463/458` 基线逐项相同：真实 Battle 动态果冻数量、`5503` 精英 0 旧技能描述，以及三项 Windows CRLF 敏感的 `UnitSourceConsumerEditModeTests`；失败列表没有 Match 测试，因此全量不能记为通过，但没有本次 M3 新增失败。
- Unity 导入曾只把 `Assets/Resources/Characters` 的空 YAML 值改为空格并改写 code-coverage 设置；确认这些是测试产生的无关序列化噪声后，已按精确路径恢复到 M2 基线。最终 diff 不包含这些资源或 ProjectSettings。
- 本轮没有运行 PlayMode、Windows/Android 构建、Socket、场景、UI、设备或真实 LAN 验证。M3 是 `noEngineReferences=true` 的纯 C# 领域扩展；M4—M8、牌库外 UnitId 生成、Buff 数值效果和 BattleInput adapter 均未实现。

## 55. LAN Match M4 配对、阶段、结算与淘汰（2026-07-30）

- 实现分支为 `codex/lan-match-flow`，独立 worktree 为 `G:\ARKnoNIGHTS_beta\.worktrees\lan-match-flow`。M1 实现/文档基线分别为 `861f198` / `c615a752c324c836b18dfabfbe553943b5f02f5d`，M2 为 `1d62ee713aa17ce24d2bc2098516d9c7eea55fed`，M3 直接基线为 `a1e3f12559b24b8275a70a1148b403634b232bfe`。没有修改 Scene、Prefab、Package、ProjectSettings、Lobby Socket、Battle Core、PlayerState 或表现资源。
- 修改前 focused Match EditMode 使用 Unity `D:\2022.3.62f1c1\Editor\Unity.exe`、项目脚本 `scripts/Invoke-UnityTests.ps1` 和过滤器 `ArknoNights.Match.Tests`，结果为 `87 total / 87 passed / 0 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`；证据位于 `Temp/UnityTests/M4-Baseline-20260730-1`。
- M4 focused 测试过滤器为 `ArknoNights.Match.Tests.MatchFlowEditModeTests`。最终定向结果为 `26 total / 26 passed / 0 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`；证据位于 `Temp/UnityTests/M4-Flow-20260730-9`。用例覆盖 30 秒单调时钟及跨结算时钟连续性、最后 Ready 原子封印/Fatal 提交、掉线宽限、权威阵型、首回合安全购买/部署、四/三/两人黄金配对表、四人淘汰为三人时的确定性候选选择/SeatIndex 决胜/诊断、结果 hash/Outcome 校验、回放门控、影子映射、结算输出审计/收入/淘汰/排名、NoContest、legacy API 隔离、隐私和幂等。
- 第一次整合 M1–M4 的完整 Match 回归定位到旧 M1–M3 主机 phase 原语仍会显式进入 `Sealing`；修正为离开 Preparation 时关闭准备时钟后，相关既有用例恢复。随后一次 `M4-Final-Match-20260730-3` 在 NUnit XML 写入完成前触发 wrapper 读取竞态，被明确标记为 `unverified / forced-stop-before-valid-results`，不计为通过或失败。确认无残留 Unity 进程后使用新目录重跑。
- 有效的最终 Match focused 命令为 `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\Invoke-UnityTests.ps1 -UnityPath D:\2022.3.62f1c1\Editor\Unity.exe -ProjectPath G:\ARKnoNIGHTS_beta\.worktrees\lan-match-flow -TestPlatform EditMode -TestFilter ArknoNights.Match.Tests -OutputDirectory Temp\UnityTests\M4-Final-Match-20260730-6 -TimeoutSeconds 900 -NoGraphics`。结果为 `113 total / 113 passed / 0 failed / 0 skipped / 0 inconclusive / 0 not-run / 0 not-runnable`；XML 与日志分别为该目录的 `EditModeResults.xml` 和 `EditMode.log`。完整 XML 写出后 wrapper 执行 `forced-stop-after-results`，不是超时或结果缺失。
