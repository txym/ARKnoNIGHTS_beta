...
> 本文件保留按任务追加的架构演进说明，不再作为当前架构入口。
> 当前实现边界请阅读 [`docs/ARCHITECTURE.md`](../ARCHITECTURE.md)。
...
- `LocalMatchState` 为每个 `LocalMatchPlayerData` 持有独立的等级和六槽商店，快照为四名玩家分别投影商店；购买、冻结、升级和付费主动刷新仍只定位 `LocalPlayerId`。`ShopOfferGenerator` 在领域层按 1—9 级固定权重抽取稀有度，并在当前 Player-safe fixture 的显式 `shopTypeIds` 可购买候选池中等权抽取；加载器只读兼容旧 `shopPages`，但不再把页面边界带入领域状态。缺少候选的稀有度通过只保留可用稀有度权重实现条件重抽。`shopRandomSeed` 只保证本地 fixture 可复现。`ShopOfferOrdering` 随后按目录稀有度、十进制数值 `typeId` 和生成顺序执行稳定排序，UI 继续只按稳定 `ShopSlotId` 显示 `snapshot.LocalPlayer`，不保存第二份生成或排序状态。
- `PreparationBattleLoopController.ReturnToPreparation()` 在 `MultiBattlePresentationState.Completed` 后、清空 multi-battle observation map 前调用一次 `RefreshAllShopsAfterBattle()`；该同步操作按每名玩家当前等级生成商品、保留各玩家冻结槽、刷新其余槽并只发布一次完整四玩家快照。`ResetPreparationUiState()` 仍只负责 ready 与观察目标，不负责商店刷新。共享卡池副本扣留、购买占用与刷新返池尚未接入这一领域边界。
...
- `FormalBattleHudController.SetSessionHudValues` 只显示 loop-owned 本地赤金、生命和下一对手名称。当前商店概率测试 fixture 因此显示赤金 `200`、生命 `400`；`200` 是人工测试额度，不代表正式经济初始值，生命扣除、淘汰及完整经济规则仍未实现。
...
- 同一套 `Timed/Summon` 数据流允许 `sideLengthCentimetres=0` 表示精确使用施法者中心，只有负值非法；`SUMMON_REPAIR_HELPER` 由 `10077` 在该中心生成一个 `10073`。Spawn 仍与 Skill 同 Tick，动态实例在下一 Tick 激活并进入普通索敌，不继承施法者路径、目标或阻挡关系。
- `Timed` 能力还可携带一个目标范围伤害效果，与 Summon 互斥。施放前先按地面资格、距离和稳定决胜选择主目标；没有目标时不消费 SP。施放时封存有效攻击力与主目标 ID，到完整二倍速 Skill 动画到期 Tick 以主目标当前位置展开圆形范围；主目标失效会取消该次结算，各范围目标读取到期时防御、法抗和承伤修正。该伤害不是普通攻击，不进入普通攻击专属闪避、反伤、命中效果或攻击计数链。
...
- 被动能力可携带持续或首次触发的整数生命阈值战斗修正。阈值比较以千分比交叉相乘完成，可组合攻击/防御/移速倍率、阻挡与攻速加算，也可在精确的限时 Tick 区间内把阻挡容量强制为0；状态在 Tick 开始、同批直接伤害之后及持续生命变化之后刷新，同批伤害不会因单位遍历顺序而部分使用新阈值。阻挡容量降低时按 unit ID 稳定释放超出新容量的关系。阈值触发的动画后回满效果先进入实例私有队列；已经开始的 Attack 完整出伤后才发出二倍速 Skill 事件，并在动画有效 Tick 到期时产生一次 `HealthChanged`。未被阻挡条件减伤在每次实际伤害结算时读取对称阻挡关系，仅修正物理/法术伤害。攻击序列修正在攻击成功开始时推进序号，并把该次快照攻击力与破防倍率封存在待结算攻击中；一次性/固定间隔增强和指定攻击前的永久状态切换均不受目标随后死亡影响。攻击计数状态可在自身转换时强制同阵营其他计数状态解放；传播后开始的攻击读取新状态，已开始攻击保持快照。范围攻击同样由封存序号决定，伤害 Tick 再以主目标当前位置扩展圆形半径或最近格十字区域；每个范围目标独立进入普通攻击伤害、闪避和反伤链。未阻挡攻击蓄力按实例激活 Tick 的固定间隔增长，作为全部攻击倍率后的最终固定加算进入 Attack 快照，并在该次待结算攻击到期时统一清零。特殊部署接近特征会把初始单位的封存部署位置保存为目的地，并将实际 Spawn 位置替换为本方门格；单位以既有定点移动到达后驻留，依赖该来源的 Aura 在途中保持关闭、到达 Tick 的移动后光环刷新阶段才启用。`1146` 的 Aura 使用 `250cm` 闭区间半径：基础层提供防御 `+200`、回复 `400/s`，精英 2 再提供防御 `+100`、回复 `100/s`；两层分别按 ability ID 去重，多个来源只扩展覆盖而不重复叠加数值。
- 死亡后继能力在 Death 产生时封存所属玩家、阵营、位置、ability ID 与到期 Tick；延迟期间 pending successor 会维持该阵营的战斗存续，避免在后继体出现前错误判胜。到期项按 Tick、死亡单位 ID、ability ID 稳定处理，候选类型按权重哈希选择，位置偏移和动态负 ID 均可重放；Spawn 仍沿用 `ActivationTick = SpawnTick + 1`。延迟死亡范围伤害同样在 Death 封存阵营、位置、有效攻击力和效果，并维持终局待定；到期时按目标聚合同 Tick 伤害，读取目标当时的防御、法抗和承伤修正，但不触发普通攻击专属的反伤或闪避。当前 Core 没有地形图，“最近可通行格”仅在 9×8 非门格中按距离、X、Y 决胜；单位数据没有独立空中命中标签，也不需要为 `1089` 增加该标签，因为无人机已由公共目标资格从全部伤害候选排除。
- 外部数值修正按每 Tick 开始位置计算，并在移动、阻挡评估和死亡清理后刷新；范围使用厘米整数平方距离，支持全场/半径、敌我、排除来源、阻挡对手、同类型邻近计数和按 ability ID 不叠加。贡献按来源 unit ID、ability ID 排序后应用到攻击/防御/法抗/攻速/移速/回复，死亡来源同 Tick 后续阶段不再提供效果。Aura 还可向有效目标授予瞬时状态标签，目标的条件战斗修正按当前标签集合生效；标签来源死亡、离场或离开范围后同次刷新移除。物理/法术攻击闪避在伤害 Tick 以 battle ID、双方 unit ID、ability ID、伤害 Tick 和已封存攻击序号进行稳定千分位判定，成功时保留零伤害事件；真实伤害及非攻击伤害不进入该判定。命中后持续生命流失仅由正伤害普通攻击施加，目标按 ability ID 保存并刷新失效 Tick；生命周期阶段将其与其他每秒生命变化合并为整数余量 `HealthChanged`，不递归进入攻击反应链。其他异常状态尚未进入 Core。
- Tick 在处理本 Tick 伤害、Death、阻挡解除和目标清理后判定终局；终局 Tick 不恢复 SP、不施放技能。正伤害普通攻击可按攻击序号或存活目标的受击次数触发生成；多目标攻击按原始攻击者+攻击序号去重，受击按稳定 Hit 顺序计数，生成前可检查同阵营同类型存活上限。成功攻击触发可为对应序号配置 Skill 攻击动画；该次攻击以带目标的 Skill 事件替代 Attack 事件，使用二倍速向上取整后的完整动画时长封存伤害 Tick，并在正伤害结算后进入原触发生成流程。一次性生命阈值生成会把 owner 当 Tick 位置映射到最近战场格，并在有效且非门的四个正交相邻格中心建立实例。非终局 Spawn 只创建无路径、无目标、无阻挡继承的新实例，`ActivationTick = SpawnTick + 1`，之后按普通单位规则重新索敌。
- 正伤害普通攻击还可携带目标实例型永久防御 Debuff。`RuntimeUnitState` 只保存饱和累加的减防总量；同一目标伤害组先用组开始时防御计算全部 Hit，随后按攻击者 ability ID 稳定顺序应用每个正伤害 Hit 的减防。有效防御在既有自身/状态/光环修正之后扣减并限制到 `0`，攻击者死亡或离场不会清除目标实例上的累计值。
- 通用持续被动战斗修正可组合阻挡、法抗和攻速加算及物理/法术承伤千分比；相同效果的多个单位通过稳定 ability ID 共享定义。生产源现已接入 `1058/1081 +2` 阻挡、`1240 +1` 阻挡、`1165/1166/1170 +70` 法抗、`1230 +60` 法抗，以及 `10127 +100` 攻速与两类承伤 `×500/1000`。`FORTIFIED_CATERING_VEHICLE` 仍由 `10038/10039` 共用，提供阻挡 `+2` 与两类承伤 `×100/1000`；真实伤害不受承伤千分比影响。
- 被动攻击位移效果按已开始攻击序号触发，并把攻击快照、原目标、位移到期 Tick 和距离封存到 Core。触发 Tick 先解除全部阻挡并临时把有效阻挡容量置零；首个技能片段完整二倍速播放后，Core 用整数向量朝敌方门移动并在同 Tick 通过既有 `PendingAttack` 结算普通攻击。整段 Skill 序列继续持有技能锁，因而出伤后也不能提前开始下一击。技能动画目录允许把多个稳定 key、真实名称与逐片段源 Tick 组成有序序列；`UnitSkelPresentationView` 在 Spine 0 轨以 `2×` 逐段排队播放。`GREY_HAT_THIRD_ATTACK_DASH` 以此表达 `1322` 的第三击、`150cm` 位移、`20 Tick` 不可阻挡及 `Skill_Begin → Skill_Loop → Skill_End`。
- Timed 闪现效果把“当前被阻挡”作为施放资格；资格不满足时不消费已满 SP。施放仍通过统一 Attack/Skill 锁门控，随后解除阻挡、建立覆盖完整有效 Skill 区间的临时不可阻挡状态，并把首片段到期的 `PendingRelocation` 排入确定性队列。到期项先于本 Tick 普通移动和门区判断执行，因此权威位置、后续索敌及同 Tick 冲门退出使用闪现后位置。`BLOCKED_BLINK_FORWARD` 以此表达 `1502` 的 `Disappear → Appear`、`10 Tick` 二倍速技能和前摇后 `150cm` 迁移。
- 被动进入半径伤害在普通移动、冲门退出及移动后光环刷新完成后，按来源 unit ID、ability ID、目标 unit ID 稳定扫描。每个 `RuntimeAbilityState` 保存当前范围内目标 ID 集合，仅为集合新增项生成 Hit；离开项会从状态移除，从而允许重新进入后再次触发。同 Tick Hit 按目标聚合、批量扣血后再统一清理死亡，不进入普通攻击专属反应链。`GROUND_PROXIMITY_COLLISION_DAMAGE` 以此表达 `10038` 的 `50cm` 地面碰撞与攻击力 `100%` 物理伤害。
- `TriggeredSpawn` 能力源由 `AbilityCatalogGenerator` 投影触发类型、首次/重复序号、召唤类型、方形边长与同类存活上限；`AbilityCatalogLoader` 再从独立技能动画目录为成功攻击触发补入逻辑动画 key 和源 Tick。`2031/2033` 因此直接读取各自 v2 `attack.skill → Skill / 2s`，Core 统一按 `40 → 20 Tick` 播放第三击、在正伤害后生成 `2033`，并复用既有确定性方形位置、动态 ID 与下一 Tick 激活规则。项目不存在路径点，触发生成不保存或继承路径对象。
- `HealthThresholdCombatModifier` 与 `HealthThresholdAdjacentSpawn` 能力源分别投影首次/持续阈值战斗修正和一次性四邻格召唤；运行时加载器据非零阈值字段恢复对应 Core 定义。`10006` 用两条单效果被动共同表达首次严格低于 `50%`：四个非门正交格中心各生成一个 `10002`，同时激活永久 `2500/1000` 移速倍率。相同战斗修正投影还直接服务 `1025` 的精英分级持续攻击倍率、`1232` 的持续防御/阻挡、`1264` 的首次受伤后 `300 Tick` 加速与 `1274` 的首次半血永久加速；每条状态独立确定性更新。需要永久表现切换的阈值或攻击计数状态通过 `PresentationStateChanged` 进入 Track；技能动画仍按统一二倍速规则等待当前 Attack 结束，状态标签则在配置的动画到期或指定攻击开始前生效。Skill 动画目录同时保存状态对应的待机、移动、攻击和死亡 Spine 动画名，Track seek/replay 会恢复当时状态，不回写 Core。
- 目标资格由攻击方的 `AttackMethod` 与目标的被动特征共同判定；`Untargetable` 与历史能力 ID `UntargetableByMelee` 都把无人机从全部敌方索敌及伤害候选中排除。可移动单位没有合法攻击目标但敌方仍有在场实体时改向对方门移动。门区用门中心上下左右各 `40cm` 的闭区间正方形判定；进入后实体保留终局快照但标记 `HasExitedBattle`，清理目标、阻挡和待结算攻击，并发出 `GateReached`。Track 在该 Tick 起停止创建或立即销毁对应视图，不播放 Death。单场 Core 分别累计冲门 `lifeDeduct` 与终局仍存活敌方实体的 `lifeDeduct`，输出 `HomeLifeLoss/AwayLifeLoss`，自然终局以生命损失较少方为唯一胜方；持久玩家生命写回仍由外层回合结算负责。
