...
> 房间加入方式采用已确认的局域网房间号发现/加入流程；人数不足时的启动规则见第 15 节。断线重连、淘汰后的行为以及胜负结算细节仍待后续补充。
...
完整对局不设置单名玩家同时存活战斗实体数的全局上限。召唤物生成后是普通战斗实体，精英化等级继承召唤源，并使用自身单位类型数据正常参与索敌、移动、阻挡、攻击、受击、死亡、冲家和目标价值结算；不得因为其来源是召唤而跳过这些流程。除塑路者分形继续遵守其能力定义中的共享数量门槛外，其他召唤物当前不设置单种召唤物共享数量上限，因此允许持续召唤能力随战斗时长继续扩大实体数和潜在玩家生命伤害。正式战斗时长仍待确认。
...
完整对局当前不设置持有赤金利息，剩余赤金不会因为跨回合保留而自动产生额外赤金。单场战斗固定奖励是否存在，以及未来是否按战斗结束时待部署区中的精英二、精英三单位数额外发放赤金，均仍待确认。

当前本地 UI Demo 使用以下临时确定性配置：

- 玩家初始等级为 `1`，最高等级为 `9`；
- 完整经济设计中的正式初始赤金仍待后续实现；为了方便当前商店概率人工测试，本地测试玩家初始赤金为 `200`；
- 玩家初始生命为 `400`；
...
- 每格先按等级权重抽取稀有度，再在该稀有度的当前可用商品类型中等权抽取；当前候选集没有该稀有度时，按仍有候选的稀有度权重条件重抽，不生成池外单位；
...
当前本地 UI Demo 中，点击准备不会提前结束准备阶段，准备阶段仍在 `30` 秒倒计时结束后进入战斗。进入战斗阶段时隐藏准备按钮、关闭已经打开的商店并清除尚未完成的购买或升级确认，但保留等级按钮；玩家可以在战斗阶段再次打开自己的商店。战斗结束返回准备阶段时，准备状态重置为未准备。多人状态同步、取消准备的网络截止时间以及未来“全员准备后提前开始”的规则后续补充。
...
同一回合有多名玩家生命同时降至 `0` 或以下时，先比较扣除后的实际生命值，数值较高者排名更高；不得先把生命统一截断为 `0` 再比较。例如 `-3` 的排名高于 `-8`。若扣除后的实际生命仍完全相同，其最终决胜规则待确认。

战斗平局、战斗超时和双方战斗实体同时全灭时的结算规则暂不处理，后续补充。
