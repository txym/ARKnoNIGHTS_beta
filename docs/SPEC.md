...
### 2.4 当前正式 LAN 对局阶段（M1—M8 已实现）

M1—M8 已把既有 LAN 房间、准备/商店/部署、Battle Core 和多战斗表现连接为房主权威的正式对局。正式路径使用独立 Match 状态、原连接提升、权限化快照、自动人机、重连、每端独立战斗计算、统一播放时钟和权威结算；设计见 `docs/LAN-MATCH-DESIGN.md`，实现与验证状态见 `docs/LAN-MATCH-IMPLEMENTATION-PLAN.md` 和 `docs/history/TEST_RECORDS.md`。
...
完整对局的精英化目标规则已经确认，但当前本地 UI Demo 尚未实现：
...
- 部署 Cost 当前存在三套数据口径：`docs/bonds/BONDS_SPEC.md` 的逐单位表、v2 单位源统一为 `2` 的现状、冻结 Player-safe 目录的 `1000/5503/5504 = 2/12/4`。正式迁移前必须确认哪一套是目标权威值；
