...
> **文档状态：持续完善中（Draft，当前摘要更新于 2026-07-29）**
...
项目已经具备确定性战斗、本地四玩家准备—战斗循环、商店/部署/HUD，以及 LAN 房间发现、创建、加入、准备和开始流程；尚未实现开始游戏后的房主权威 Match Session、正式状态同步、重连、完整结算与淘汰。
...
当前 UI 接入已包含由固定本地数据驱动的临时商店、购买、刷新、冻结、升级和赤金变化；这些规则用于 Demo 闭环，不代表最终经济平衡。玩家生命扣除、完整回合结算和多人功能仍未实现。

### 2.4 当前正式 LAN 对局阶段

当前下一阶段目标是把既有 LAN 房间、固定本地四玩家状态、准备/商店/部署、Battle Core 和多战斗表现连接为房主权威的正式对局。持续收敛中的设计见 `docs/LAN-MATCH-DESIGN.md`，实施顺序见 `docs/LAN-MATCH-IMPLEMENTATION-PLAN.md`；开始对应里程碑前仍需确认设计状态并回写冲突。

在该阶段完成前，必须保持以下边界：

- Lobby 的“开始游戏”只代表解除当前本地 Demo 门控，不代表网络成员状态已经成为对局权威源；
- 每台设备独立加载的 `local-match-state-v1` 不能作为正式同步结果；
- 玩家生命、共享卡池、经济、配对、淘汰、AI 接管、重连和最终胜负必须由未来 Match 层统一结算；
- 不得以当前本地 fixture 或界面表现替代跨端一致性、命令确认和权威快照验证。
...
正式 Match M2 纯领域层已实现以下完整对局规则，但尚未接入当前本地 UI Demo、Lobby 或 LAN 传输：
...
完整联网对局中，当前在线且未淘汰的全部真人准备后立即封存并进入战斗，不继续等待倒计时；AI 没有准备状态。最后一名真人的 Ready 被房主接受后阶段立即转换，不能再取消。尚未全员准备时可以取消准备。处于掉线宽限期的真人按未准备计算，只阻止提前开始，不阻止 `30` 秒硬上限；准备阶段掉线会清除 Ready，重连后必须重新准备。当前本地 UI Demo 在联网状态机接入前仍保持倒计时结束才进入战斗的旧实现，不能把该现状误写为完整对局规则。

### 5.3 从准备阶段进入战斗阶段

完整联网对局在 `30` 秒倒计时归零或全部当前真人 Ready 时进入战斗，以先发生者为准。当前本地 UI Demo 尚未接入该多人权威转换。
...
- 房主权威 Match、版本握手、命令排序、状态快照和跨端摘要一致性；
- 断线宽限、重连 token、主动退出、AI 接管和房主崩溃后的会话结束；
- 共享卡池副本扣留/返还、正式经济、跨回合玩家生命写回、淘汰、最终胜负与对局结束；
- 完整精英化/合成运行时、Overflow 与正式 Player-safe 全单位目录迁移；
- 正式目标平台构建、Windows/Android 同 Wi-Fi 实机流程和多分辨率人工验收。
