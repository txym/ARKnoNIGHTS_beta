...
- 房间创建、加入和开始条件；
- 多人状态同步及权威端设计；
- 断线、重连和玩家退出的联网判定、状态迁移及对观察、配对、生命和持有单位的业务影响；
- 商店刷新、价格和购买规则；
...
## 16. LAN 房间统一立绘边框契约与当前验收状态（2026-07-28，当前权威补充）

本节补充第 15 节的房间页视觉规则。运行时为兼容既有清单与调用方仍使用节点/属性名 `CardBody`，但其玩家可见语义是 `PortraitFrame`（立绘区域边框）。
