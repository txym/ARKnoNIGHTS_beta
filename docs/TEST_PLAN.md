...
6. 视觉差异报告：先由可见 `-force-d3d11` Windows Player 在 `1920x1080` 写入五个 `LanLobbyCaptureSuite` 状态到忽略的 `Temp/LAN-LOBBY/CapturesFinal/`；再运行 `scripts/ExportLanLobbyVisualDiff.ps1`，在隔离 worktree 中必须显式传入只读的 `G:\ARKnoNIGHTS_beta\docs\references\ui\battle_hud`。导出只允许写入忽略的 `Temp/` 或 `Artifacts/`，必须核查 JSON 的原始参考尺寸、独立 X/Y 归一化、命名遮罩、至少一项 `ATTENTION`（仅信息性）以及每个渲染 Sprite 的 Resources 路径、素材库相对路径、SHA-256 和出现次数。不得因视觉指标改变 UI 来获取更低差异；默认参考目录不含精确图9/图10、错误尺寸截图、未映射 Sprite 或不安全输出路径必须在输出目录创建前失败。
