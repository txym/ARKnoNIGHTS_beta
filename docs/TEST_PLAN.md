...
## Home room-select action-bar retained Player evidence (2026-07-26)

1. Run these suites sequentially with `scripts/Invoke-UnityTests.ps1`, keeping their output directories distinct: `ArknoNights.Lobby.Tests.LanLobbyLayoutEditModeTests` to `Temp/ROOM-SELECT-ACTION-BARS/FinalLayout` (EditMode), `LanLobbyViewPlayModeTests` to `FinalView`, `LanLobbyCaptureSuitePlayModeTests` to `FinalCapture`, and `LanLobbyControllerPlayModeTests` to `FinalController` (the latter three PlayMode). Each output XML must be valid, have non-zero `total`, `result=Passed`, and `failed=0`. The retained 2026-07-26 run is 6/6, 13/13, 3/3, and 3/3 respectively (25/25 total).
