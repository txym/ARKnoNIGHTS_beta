...
## 36. Mainline 果冻召唤最终集成验证（2026-07-26）

- 所有证据保存在 `.superpowers/sdd/2026-07-26-mainline-jelly-summon/evidence/task5/`。基线命令为：
  - `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\Invoke-UnityTests.ps1 -UnityPath D:\2022.3.62f1c1\Editor\Unity.exe -TestPlatform EditMode -ProjectPath G:\ARKnoNIGHTS_beta\.worktrees\summon-jelly-minions-v2 -OutputDirectory .superpowers\sdd\2026-07-26-mainline-jelly-summon\evidence\task5\baseline-edit -TimeoutSeconds 1800`
  - `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\Invoke-UnityTests.ps1 -UnityPath D:\2022.3.62f1c1\Editor\Unity.exe -TestPlatform PlayMode -ProjectPath G:\ARKnoNIGHTS_beta\.worktrees\summon-jelly-minions-v2 -OutputDirectory .superpowers\sdd\2026-07-26-mainline-jelly-summon\evidence\task5\baseline-play -TimeoutSeconds 1800`
  - EditMode 基线 XML 为 `baseline-edit/EditModeResults.xml`：`159 total / 158 passed / 1 failed / 0 skipped`；失败项是旧 Replay 断言用最终 `49` 个实例乘两次，忽略第二次 Replay 启动时只创建 Tick `0` 的 `7` 个 authored views。PlayMode 基线 XML 为 `baseline-play/PlayModeResults.xml`：`22 total / 21 passed / 1 failed / 0 skipped`；失败项把最终动态实例也要求为 Tick `0` 已有 GameObject。两项均为已证明的旧测试假设，不是 Core 行为失败。
- 定向回归命令在 `-TestFilter` 分别使用 `ArknoNights.Battle.Tests.BattleDemoCoordinatorEditModeTests;ArknoNights.Battle.Tests.PreparationBattlePhaseEditModeTests` 与 `ArknoNights.Battle.Tests.BattleDemoCoordinatorPlayModeTests;ArknoNights.Battle.Tests.PreparationBattleLoopPlayModeTests`。`focused-green-edit/EditModeResults.xml` 为 `8/8` 通过；PlayMode 首次 `focused-green-play/PlayModeResults.xml` 为 `3 total / 2 passed / 1 failed`，证明同帧检查碰到了 Unity 延迟销毁对象；等待一帧后 `focused-green-play-rerun/PlayModeResults.xml` 为 `3/3` 通过。回归覆盖 Tick `0` authored 数量 `7`、Tick `100+` 动态 `5504` 数量 `42`、动态视图只在越过 Spawn Tick 后出现、Replay 清理，以及两个准备入口均封存 `SUMMON_JELLY_MINIONS`。既有 `BattleInput_RejectsUnitWithUnknownInnateAbilityId` 继续验证未解析 ability ID 的结构化失败。
- 最终全量命令与基线相同，仅将输出目录替换为 `final-edit` 和 `final-play`。`final-edit/EditModeResults.xml` 为 `159 passed / 0 failed / 0 skipped`；`final-play/PlayModeResults.xml` 为 `22 passed / 0 failed / 0 skipped`；两次均产生非零 XML 并正常退出。最终日志扫描未发现 `error CS`、`Compilation failed`、`Scripts have compiler errors`、`Unhandled Exception`、`NullReferenceException` 或 `AssertionException`。
- 独立编译命令为 `D:\2022.3.62f1c1\Editor\Unity.exe -batchmode -nographics -quit -projectPath G:\ARKnoNIGHTS_beta\.worktrees\summon-jelly-minions-v2 -logFile G:\ARKnoNIGHTS_beta\.worktrees\summon-jelly-minions-v2\.superpowers\sdd\2026-07-26-mainline-jelly-summon\evidence\task5\final-compile.log`，退出码 `0`，日志以 `Exiting batchmode successfully now!` 和 return code `0` 结束，未发现 C# 编译错误。
- 未验证：本轮未执行目标平台构建，也未在交互式 Unity Editor/Windows Player 中人工目检三个 `5504` 的一格正方形分布、Tick `100/250` 出现瞬间、各自索敌、死亡清理、Home/Away 投影、暂停/变速及 Replay 视觉连续性。自动 Core/Presentation/PlayMode 断言和 batch compile 不替代这些视觉检查。
