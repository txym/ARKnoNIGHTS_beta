...
> 状态：历史规划。第一阶段战斗闭环已经形成当前实现基线；本文保留原任务边界和验收设计，不作为当前能力摘要。
...
- 目标死亡后的下一 Tick 才重新索敌。
- 阻挡为对称关系；全局阻挡半径为 `0.4` 米，双方距离严格小于该值时才满足范围判定，单位阻挡容量默认 1。
- 阻挡判定完成后，同一 Tick 可以开始攻击；攻击不强制要求已建立阻挡，但目标必须满足同一个严格小于 `0.4` 米的距离判定。
