...
- 被动能力可携带持续或首次触发的整数生命阈值战斗修正。阈值比较以千分比交叉相乘完成，可组合攻击/防御/移速倍率、阻挡与攻速加算；状态在 Tick 开始、同批直接伤害之后及持续生命变化之后刷新，同批伤害不会因单位遍历顺序而部分使用新阈值。持续阻挡加成失效时按 unit ID 稳定释放超出新容量的关系。未被阻挡条件减伤在每次实际伤害结算时读取对称阻挡关系，仅修正物理/法术伤害。
