...
Match wire 使用独立 `MatchWireEnvelope` 和 4 字节大端长度前缀：绝对上限 4 MiB，控制消息 64 KiB，完整 scoped snapshot 1 MiB，BattleSeal 为 4 MiB 减前缀。协议使用严格 UTF-8、显式 DTO/方向/范围校验，不依赖运行时类型名。每次权威事务先向发送者排队 `CommandAck`，再为各在线 Human 独立投影并序列化 Public + 本人 OwnerPrivate 完整快照；客户端只原子接受更高 `StateRevision`。
...
M6 只提供 BattleSeal、FirstChunkReady、PlaybackStart、PlaybackClock、FinalSecondHash 和 ClientBattleFailure 的严格 transport/恢复顺序，并为未来 M5 提供连接丢失、恢复 Human 和显式退出 sink；不生成 BattleChunk、checkpoint 或 hash，也不实现 AI 策略。

### 3.4 Match M1–M5 纯领域权威状态与 AI
...
权限投影保持三层：Public 增加权威剩余时间、公开配对、生命、淘汰、名次与安全终局信息，但不含商店、赤金、等级、Overflow、tombstone、封印 hash/seed 或玩家层定向 Buff payload；OwnerPrivate 只含仍存活的本人完整状态，淘汰旁观者只获得 Public；HostMatchSnapshot 额外包含完整 `MatchFlowState`、池、封印计划、结果、结算记录与诊断。Lobby Session 消费 Match 权威命令与投影；PlayerState、Round 和 Battle Demo 仍不是联网权威状态源。
...
- M5 AI 已合入，但尚未由 M6 Session 的正式运行时组合根创建、调度和接管；
- 正式 Battle Core adapter、流式计算/回放与 LAN 多端闭环；
- M7 的 100 Tick BattleChunk、checkpoint、本地 hash 计算、等待/错配协调；
- M8 正式 Match HUD、场景全面接线和结算界面；
- 主机迁移、专用服务器与反房主作弊；
- 目标平台发布配置以及全面的分辨率、设备和人工视觉验收。

不能用本地 Demo 的四玩家 fixture 代替正式联网状态，也不能把 M6 的 Battle transport 契约描述成已经实现 M7 Core/Chunk 内容。
