...
- `HealthThresholdCombatModifier` 与 `HealthThresholdAdjacentSpawn` 能力源分别投影首次/持续阈值战斗修正和一次性四邻格召唤；运行时加载器据非零阈值字段恢复对应 Core 定义。`10006` 用两条单效果被动共同表达首次严格低于 `50%`：四个非门正交格中心各生成一个 `10002`，同时激活永久 `2500/1000` 移速倍率。两条状态各自确定性触发一次，不依赖 Skill 动画或表现回调。
