...
- 数值光环按每 Tick 开始位置计算，并在移动和死亡清理后刷新；范围使用厘米整数平方距离，支持全场/半径、敌我、排除来源和按 ability ID 不叠加。贡献按来源 unit ID、ability ID 排序后应用到攻击/防御/法抗/攻速/移速/回复，死亡来源同 Tick 后续阶段不再提供光环。沉默状态与标签传播尚未进入 Core。
