...
>
> 本文只描述仓库中已经存在的结构和能力，不把规划中的正式 LAN 对局写成已实现。游戏规则以 [`SPEC.md`](SPEC.md) 为准；历史演进见 [`history/ARCHITECTURE_EVOLUTION.md`](history/ARCHITECTURE_EVOLUTION.md)。
...
- 当前产品形态：单场景本地四玩家自走棋 Demo，加上可用的 LAN 房间流程；正式同步对局尚未接入。
...
第一方 EditMode 和 PlayMode 测试分别位于 `Assets/Game/Tests/EditMode` 与 `Assets/Game/Tests/PlayMode`，按 Battle/Lobby 分程序集。
...
- 房主权威 Match 领域、正式命令协议、Match 快照和兼容性握手；
...
- 改联网：遵守 `Lobby → Initial 集成 → 本地门控` 的现有依赖，正式 Match 按 LAN 设计新增独立领域层。
