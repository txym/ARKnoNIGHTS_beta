...
- `HealthThresholdCombatModifier` 与 `HealthThresholdAdjacentSpawn` 能力源分别投影首次/持续阈值战斗修正和一次性四邻格召唤；运行时加载器据非零阈值字段恢复对应 Core 定义。`10006` 用两条单效果被动共同表达首次严格低于 `50%`：四个非门正交格中心各生成一个 `10002`，同时激活永久 `2500/1000` 移速倍率。相同战斗修正投影还直接服务 `1025` 的精英分级持续攻击倍率、`1232` 的持续防御/阻挡、`1264` 的首次受伤后 `300 Tick` 加速与 `1274` 的首次半血永久加速；每条状态独立确定性更新，不依赖 Skill 动画或表现回调。
- 目标资格由攻击方的 `AttackMethod` 与目标的被动特征共同判定；`Untargetable` 排除全部正常索敌，`UntargetableByMelee` 只排除近战索敌。可移动单位没有合法攻击目标但敌方仍有在场实体时改向对方门移动。门区用门中心上下左右各 `40cm` 的闭区间正方形判定；进入后实体保留终局快照但标记 `HasExitedBattle`，清理目标、阻挡和待结算攻击，并发出 `GateReached`。Track 在该 Tick 起停止创建或立即销毁对应视图，不播放 Death。单场 Core 分别累计冲门 `lifeDeduct` 与终局仍存活敌方实体的 `lifeDeduct`，输出 `HomeLifeLoss/AwayLifeLoss`，自然终局以生命损失较少方为唯一胜方；持久玩家生命写回仍由外层回合结算负责。
