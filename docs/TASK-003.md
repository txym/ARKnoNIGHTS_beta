...
4. 实现 `0.4` 米半径内的对称阻挡、默认容量 1、移动停止、关系解除和死亡清理。
...
- 阻挡关系是对称的；全局阻挡半径为 `0.4` 米。
- 阻挡容量默认 1。正式第一阶段 fixture 必须规避多个单位同 Tick 竞争同一容量的情况。
- 单位与当前目标的距离严格小于 `0.4` 米且容量可用时建立关系；距离恰好等于 `0.4` 米时不建立阻挡。已阻挡双方停止相向移动。
- 一个单位碰到“容量已满且不是自己当前目标”的单位时，不因此停止移动。
- 阻挡判定完成后，同一 Tick 可以开始攻击。
- 攻击不要求已经建立阻挡关系，但目标必须满足同一个“距离严格小于 `0.4` 米”的范围判定并且攻击冷却完成。
...
3. 阻挡：距离 `<0.4` 建立、距离 `=0.4` 不建立、对称关系、容量 1、非目标满容量单位不停车、死亡双向清理；
