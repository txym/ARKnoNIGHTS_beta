...
本文描述战斗界面的待部署区（`StagingArea`）、单位槽位（`StagingSlot`）、资源显示区和其他 HUD 元素的基础尺寸、排列与视觉状态。
...
当前第一阶段不包含完整的玩家生命扣除规则，因此本节只确定布局用途和贴图来源，不把玩家生命变化描述为已经实现。生命初始值、扣除条件和显示格式仍需在相应游戏机制确认后补充。未接入占位文本使用参考图采样的浅红色 `#FF7878`。
