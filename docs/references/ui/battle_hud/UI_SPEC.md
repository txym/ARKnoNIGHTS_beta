...
| 商店面板 | `(820, 600, 1070, 280)` |
| 升级按钮（商店局部） | `(129, 78, 111, 175)` |
| 第一个商品槽位（商店局部） | `(248, 78, 158, 175)` |
| 后续商品槽位（商店局部） | `x = 248 + 166 × index`，`index = 1..4` |
| 冻结按钮（商店局部） | `(744, 0, 149, 77)` |
| 刷新按钮（商店局部） | `(902, 0, 147, 77)` |

五个商品槽位以 `8px` 间距排列，最后一个槽位右边缘等于商店面板右边缘；升级按钮与第一个商品槽位之间同样保留 `8px`，因此升级按钮属于商品组左侧，而不是商店面板左边缘。商品头像和图标保持宽高比，不为了填满上半区域做非等比拉伸。

商店商品名及冻结、刷新、准备按钮的紧凑中文标签使用仓库内 `Assets/Resources/Fonts/HanYiCuHeiJian-1.ttf`。同条件 Windows Player 截图比较表明，该字体在当前 `16～24px` 尺寸下比 `FangZhengHeiTiJianTi-1.ttf` 更接近图7和图8的字重，因此本阶段不新增或下载字体。等级、商品价格、升级价格、刷新价格和玩家生命等纯数字使用 `Novecento wide`。当前文字颜色为：
...
升级价格使用与商品价格相同的 `cost_bg_1.png` / `cost_bg_2.png` 视觉语言。升级与商品价格背景的局部矩形分别为 `(33.5, 157.5, 44, 35)` 和 `(57, 157.5, 44, 35)`；两者的中心均落在对应卡片的水平中线和上边缘。达到最高等级后隐藏升级价格背景并显示 `MAX`。
...
当前 `1920 × 1080` 参考矩形为 `(1710, 400, 180, 60)`。`ready_bg.png` 和 `ready_frame.png` 普通拉伸到完整按钮矩形；图标局部矩形为 `(18, 15, 30, 30)`，文字局部矩形为 `(48, 7, 124, 46)`，两者的垂直中心均为 `30`。
...
玩家列表（`PlayerListPanel`）固定在界面左侧，当前本地 Demo 固定显示 `4` 名测试玩家，整体排列和尺寸比例参考图7。每名玩家的初始生命为 `400`。四名玩家共用 `bg_player_list.png` 作为最底层黑色背景；在 `1920 × 1080` 下，其左上角坐标布局为 `x=24、top=164、width=116、height=534`。四行均为 `116 × 126`，行间距 `10`，每行顶部为 `164 + 136 × index`。
...
| 头像框 | `(58, 70)` | `108 × 108` |
| 生命背景 | `(58, 36)` | `92 × 24` |
| 生命图标（生命背景局部） | `(14, 12)` | `12 × 18` |
| 生命数字（生命背景局部） | `(55, 12)` | `60 × 20` |
| 本地玩家标记 | `(28, 110)` | `32 × 32` |
