...
- 攻击力显示当前实际攻击力；
- 攻击间隔显示当前实际的两次攻击动画开始时间间隔，权威单位为秒；正式单位无 Buff 时读取由源 JSON 配置折半并转换得到的 Core 实际攻击间隔，不在 UI 中自行折半；
- 防御力显示当前实际防御力；
- 法术抗性显示当前实际法术抗性。
...
- 整数属性不添加评级字母；
- 移动速度保留表达当前数据所需的小数；攻击间隔最多显示两位小数，去除无意义的末尾零，不为了排版改变权威数值；
- 法术抗性只显示数值，不附加 `%`；
- 攻击间隔只显示数值，不附加 `s`；
...
商店面板（`ShopPanel`）在准备阶段和战斗阶段均可从玩家等级按钮打开，显示在玩家等级按钮下方并位于界面上半区域。商店不得覆盖顶部中央的阶段信息栏。商店内部从左到右排列：

1. 一个升级按钮；
2. 六个商品槽位。

商品槽位整体靠商店区域右侧排列，升级按钮位于六个槽位左侧。商店打开时，在商店面板下方显示冻结按钮和刷新按钮，视觉顺序参考图7：冻结在左，刷新在右。商店关闭时，商店面板、冻结按钮和刷新按钮一并隐藏。

玩家等级按钮始终是商店的唯一打开/关闭入口；`Assets/Resources/UI/Texture/shop/btn_open.png` 不再使用。当前经 `1920 × 1080` Windows Player 截图闭环确认的左下角坐标矩形为：

| 元素 | 屏幕或商店面板局部矩形 `(x, y, width, height)` |
|---|---:|
| 玩家等级按钮 | `(1735, 930, 130, 120)` |
| 商店面板 | `(250, 550, 1605, 420)` |
| 升级按钮（商店局部） | `(-55.5, 117, 166.5, 262.5)` |
| 第一个商品槽位（商店局部） | `(123, 117, 237, 262.5)` |
| 后续商品槽位（商店局部） | `x = 123 + 249 × index`，`index = 1..5` |
| 冻结按钮（商店局部） | `(1126, -30, 223.5, 115.5)` |
| 刷新按钮（商店局部） | `(1368, -30, 220.5, 115.5)` |

商店面板及其内部内容以此前版本的 `1.5` 倍显示。面板整体相对上一版向左移动 `10`、向上移动 `40`，因此其右边缘位于玩家等级按钮右边缘左侧 `10`，上边缘超过玩家等级按钮下边缘 `40`。六个商品槽位以 `12px` 间距排列，最后一个槽位右边缘等于商店面板右边缘；升级按钮与第一个商品槽位之间同样保留 `12px`。六槽总宽度超过面板可用宽度，因此升级按钮允许越过商店面板左边缘，但仍属于同一商店组合。商品头像和图标保持宽高比，不为了填满上半区域做非等比拉伸。

商店商品名及冻结、刷新、准备按钮的中文标签统一使用仓库内 `Assets/Resources/Fonts/FangZhengHeiTiJianTi-1.ttf`。这是本轮明确指定的字体；仓库中其他不属于商店/准备组合的中文 UI 不随之批量替换。商店随整体放大后，其内部文字字号同步按 `1.5` 倍调整；准备按钮不属于商店面板，不随之放大。等级、商品价格、升级价格、刷新价格和玩家生命等纯数字继续使用 `Novecento wide`。当前文字颜色为：

| 文本 | RGBA / 十六进制 |
|---|---|
| 玩家等级按钮数字 | `(0.157, 0.867, 0.663, 1)` / `#28DDA9` |
| 升级按钮等级数字 | `(0.388, 0.871, 0.741, 1)` / `#63DEBD` |
| 商品名 | `(0.82, 0.84, 0.82, 1)` / `#D1D6D1` |
| 价格 | `(1, 0.94, 0.72, 1)` / `#FFF0B8` |
| 冻结按钮 | `(0.03, 0.16, 0.19, 1)` / `#082930` |
| 刷新按钮 | `(0.22, 0.13, 0.01, 1)` / `#382103` |
| 准备按钮 | `(0.02, 0.18, 0.17, 1)` / `#052E2B` |
...
当前本地 UI Demo 不实现免费购买，商品槽位不创建免费商品状态，也不使用 `cost_free.png`。
...
- `ice_matte.png` 原始尺寸为 `157 × 66`，显示时随商店按 `1.5` 倍放大到 `235.5 × 99`，宽度与槽位宽度近似一致，底边与槽位底边对齐；
...
冻结按钮一次作用于全部非空商品槽位：存在任一未冻结的非空槽位时，点击后统一冻结全部非空槽位；全部非空槽位均已冻结时，点击后统一解除冻结。空槽位不参与状态判断且始终不冻结。

刷新按钮正常状态使用 `refresh_bg_normal.png` 和 `refresh_icon.png`；刷新不可用时可以使用 `refresh_icon_lock.png` 替换正常图标。刷新按钮单击后立即执行，不进入二次确认状态。刷新不可用的具体条件和是否还需改变背景颜色属于商店机制，尚待确认。

当前本地 UI Demo 刷新消耗 `1` 赤金。六个初始商品全部为 `typeId=1000`，允许重复类型；刷新使用 `docs/SPEC.md` 中确认的三页固定循环。被冻结且仍有商品的槽位保留原商品，其他槽位使用下一页对应索引的商品。

刷新按钮保留 `SetRefreshFreePresentation(bool)` UI-only 接口。启用时将刷新费用背景替换为 `cost_free.png` 并隐藏费用数字；关闭时根据当前可用状态恢复 `cost_bg_1.png` 或 `cost_bg_2.png`，同时恢复费用数字。该接口默认关闭，当前 Demo 没有业务调用方；它只改变表现，不改变刷新仍消耗 `1` 赤金、按钮可用性或任何玩家状态。

冻结和刷新按钮贴图包含下方阴影，因此图标与文字不得按整个矩形的几何中心放置。在 `1920 × 1080` 参考布局中，两个按钮整体相对商店面板局部基线下移 `30`；二者的图标和文字视觉中心仍统一为按钮局部 `y=66.75`，相对按钮矩形几何中心上移 `9`。刷新价格使用与其他价格相同的 `cost_bg_1.png` / `cost_bg_2.png`；背景相对刷新按钮的局部矩形保持为 `(77.25, 0, 66, 52.5)`，因此会随刷新按钮一起下移，且背景底边始终与刷新按钮底边重合，而不是以背景中心贴刷新按钮底边。价格文字与背景水平同心，文字矩形相对背景上移 `3`，用于补偿数字字形的视觉重心。
...
`check_frame.png` 的原始尺寸为 `111 × 175`，显示时随升级按钮放大到 `166.5 × 262.5`。`check_grad.png` 拉伸到升级按钮局部矩形 `(0, 0, 166.5, 81)`；其底边与升级按钮底边对齐，上边贴到 `level` 背景可见主体的下边缘。

升级价格使用与商品价格相同的 `cost_bg_1.png` / `cost_bg_2.png` 视觉语言。放大后的升级与商品价格背景局部矩形分别为 `(50.25, 221.25, 66, 52.5)` 和 `(84.5, 221.25, 66, 52.5)`；两者相对上一版统一下移 `15`，商品价格再左移 `1`。价格文字跟随对应背景并相对背景上移 `3`。达到最高等级后隐藏升级价格背景并显示 `MAX`。
...
| 玩家状态 | 图标 | 按钮文字 | 点击结果 |
|---|---|---|---|
| 未准备 | `ready_icon.png` | `准备就绪` | 进入已准备状态 |
| 已准备 | `icon_ready.png` | `取消准备` | 取消准备 |

当前 `1920 × 1080` 参考矩形为 `(1740, 400, 180, 60)`。按钮水平中心为 `x=1830`，与右侧赤金区和部署费用区的水平中心一致。`ready_bg.png` 和 `ready_frame.png` 普通拉伸到完整按钮矩形；图标局部矩形为 `(18, 15, 30, 30)`，文字局部矩形为 `(48, 7, 124, 46)`，两者的垂直中心均为 `30`。
...
点击准备不会提前结束准备阶段，原有 `30` 秒倒计时继续推进。进入战斗阶段时强制关闭已经打开的商店、清除购买或升级确认并隐藏准备按钮，但等级按钮保持显示；玩家可以在战斗阶段重新打开商店并执行购买、刷新、冻结、升级。战斗结束返回准备阶段时，准备状态重置为未准备。

## 14. 左侧玩家列表

### 14.1 位置和头像槽位

玩家列表（`PlayerListPanel`）固定在界面左侧，当前本地 Demo 固定显示 `4` 名测试玩家，整体排列和尺寸比例参考图7。每名玩家的初始生命为 `400`。四名玩家共用 `bg_player_list.png` 作为最底层黑色背景；在 `1920 × 1080` 下，背景保持原尺寸和位置 `x=8、top=164、width=116、height=534`，不得随玩家条目一起缩小。每个 `Player_<playerId>` 条目为 `98.6 × 107.1`，条目左侧为 `x=15.7`，相邻条目的外框间距为 `13`；四个条目作为一组在背景内纵向居中，每行顶部为 `197.3 + 120.1 × index`。只有玩家条目及其内部内容相对原 `116 × 126` 尺寸按 `0.85` 倍缩小。
...
| 元素 | 中心 `(x, y)` | 尺寸 |
|---|---:|---:|
| 玩家头像 | `(49.3, 59.5)` | `78.2 × 78.2` |
| 头像框 | `(49.3, 59.5)` | `91.8 × 91.8` |
| 生命背景 | `(49.3, 30.6)` | `78.2 × 20.4` |
| 生命图标（生命背景局部） | `(11.9, 10.2)` | `10.2 × 15.3` |
| 生命数字（生命背景局部） | `(46.75, 10.2)` | `51 × 17` |
| 本地玩家标记 | `(23.8, 93.5)` | `27.2 × 27.2` |

生命背景和数字完整位于头像区域内。生命数字使用 `Novecento wide` 数字字体，并允许文字生成器在其矩形内溢出渲染，避免 `400` 被默认行高整体裁掉。本地玩家头像左上角显示 `icon_self.png`；其左边缘与玩家头像左边缘对齐，允许上方可见主体略微超出头像框。

玩家掉线后，保留原头像，并在头像上显示 `icon_lost_connect.png`。玩家已退出时，使用 `equip_replace_avatart_bg.png` 替换原头像，同时仍在替换头像之上显示 `icon_lost_connect.png`。固定本地 Demo 以 Player3 展示掉线状态、Player4 展示已退出状态；当前只定义两者的头像表现，不定义退出是否移除玩家行或改变观察、配对、生命和持有单位。掉线图标与生命及其他状态图标之间的精细遮挡顺序，以及掉线或退出后是否允许继续观察，仍属于后续多人规则。
...
24. 左侧玩家列表人数超过当前固定四人可用高度时的滚动、缩放或截断方式。
25. 玩家掉线或已退出后是否仍允许观察、掉血背景持续时间、掉血数字或动画，以及玩家淘汰（区别于主动退出）后的头像状态。
