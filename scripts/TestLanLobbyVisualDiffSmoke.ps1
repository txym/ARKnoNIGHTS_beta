...
try
{
    ...
    # The feature worktree originally lacked the user-owned reference figures, while the
    # integrated main workspace can legitimately contain the exact default reference set.
    # Exercise the behavior that matches the current checkout instead of assuming either
    # repository state.
    $defaultReferenceOutput = Join-Path $scratch 'default-reference-output'
    $defaultReferenceDirectory = Join-Path $projectRoot 'docs/references/ui/battle_hud'
    $requiredDefaultReferences = @($figure9, $figure11, $figure12, $figure13)
    $hasExactDefaultReferences = @(
        foreach ($requiredReference in $requiredDefaultReferences)
        {
            @(Get-ChildItem -LiteralPath $defaultReferenceDirectory -File -Filter '*.png' |
                Where-Object { $_.Name -ceq $requiredReference }).Count -eq 1
        }
    ) -notcontains $false
    if ($hasExactDefaultReferences)
    {
        & $exportScript -CaptureDirectory $captureDirectory -OutputDirectory $defaultReferenceOutput | Out-Null
        Assert-True (Test-Path -LiteralPath (Join-Path $defaultReferenceOutput 'visual-diff-report.json') -PathType Leaf) `
            'default reference export must produce the JSON report when the exact reference set exists'
    }
    else
    {
        Assert-FailsWithoutOutput {
