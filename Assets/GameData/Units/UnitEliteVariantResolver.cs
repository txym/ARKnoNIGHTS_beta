...
internal static class UnitEliteVariantResolver
{
    ...
    // Temporary compile-time bridge for the v1 catalog generator. Task 3 removes
    // this call site when it migrates the generator to Resolve(source, 0).
        ...
        string context)
    {
        ...
        LegacyUnitEliteVariantsDocument document;
        try
        {
            document =
                JsonUtility.FromJson<LegacyUnitEliteVariantsDocument>(variantSourceJson);
        }
        catch (Exception exception)
        {
            throw new InvalidOperationException(
        ...
        ValidateLegacyDocument(document, context);
    ...
    private static void ValidateLegacyDocument(
        LegacyUnitEliteVariantsDocument document,
        string context)
    {
        if (document == null
        ...
        foreach (var variant in ordered)
        {
            ...
            ValidateLegacyStats(variant.stats, context, variant.minEliteLevel);
            ValidateLegacyModel(
                document.typeId,
    ...
    private static void ValidateLegacyStats(
        LegacyUnitEliteVariantStats stats,
        string context,
        int eliteLevel)
    {
        if (stats == null)
    ...
    private static void ValidateLegacyModel(
        int typeId,
        string sourceVariant,
        LegacyUnitEliteVariantModel model,
        string context,
        int eliteLevel)
    {
        if (!LegacyModelDeclared(model))
        {
            return;
        }
    ...
    private static bool LegacyCombatDeclared(LegacyUnitEliteVariantStats stats)
    {
        return stats != null
               && stats.combat != null
               && (stats.combat.maxHitPoints != 0
    ...
    private static bool LegacySharedDeclared(LegacyUnitEliteVariantStats stats)
    {
        return stats != null
               && stats.shared != null
               && (stats.shared.moveSpeedMetresPerSecond != 0f
    ...
    private static bool LegacyModelDeclared(LegacyUnitEliteVariantModel model)
    {
        return model != null
    ...
    private static void ApplyLegacy(
        UnitJson source,
        LegacyUnitEliteVariantPatch variant)
    {
        ...
        if (LegacyCombatDeclared(variant.stats))
        {
        ...
        if (LegacySharedDeclared(variant.stats))
        {
        ...
        if (LegacyModelDeclared(variant.model))
        {
            ...
            source.attackAnimationDurationSeconds =
                variant.model.attackAnimationDurationSeconds;
...
[Serializable]
internal sealed class LegacyUnitEliteVariantsDocument
{
    public string schemaVersion;
    public int typeId;
    public LegacyUnitEliteVariantPatch[] variants;
}

[Serializable]
internal sealed class LegacyUnitEliteVariantPatch
{
    public int minEliteLevel;
    public string sourceVariant;
    public int statsLevel;
    public string displayNameZhHans;
    public LegacyUnitEliteVariantStats stats;
    public LegacyUnitEliteVariantModel model;
    public string[] innateAbilityIds;
}

[Serializable]
internal sealed class LegacyUnitEliteVariantStats
{
    public LegacyUnitEliteVariantCombatStats combat;
    public LegacyUnitEliteVariantSharedStats shared;
}

[Serializable]
internal sealed class LegacyUnitEliteVariantCombatStats
{
...
[Serializable]
internal sealed class LegacyUnitEliteVariantSharedStats
{
...
[Serializable]
internal sealed class LegacyUnitEliteVariantModel
{
